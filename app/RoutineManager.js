    ...
    sortByDay = function () {
        this.courses.forEach(
            (course) => {
                ...
                course.days[1] != undefined && this.sortedByDay[course.days[1]].push(course);
            }
        )
    }
        ...
        for (const day in this.sortedByDay) {
            for (let i = 0; i < this.sortedByDay[day].length; i++) {
                for (let j = 0; j < this.sortedByDay[day].length; j++) {
                    ...
                    let isSame = this.sortedByDay[day][i].name == this.sortedByDay[day][j].name;

                    if (!isSame && isConflicted) {
                        let c1 = this.sortedByDay[day][i];
                        let c2 = this.sortedByDay[day][j];

                        let c1Name = this.sortedByDay[day][i].name + "-$-" + this.sortedByDay[day][i].section;

                        if (this.conflicts[c1Name] == undefined) this.conflicts[c1Name] = [c2];
    ...
    removeConflictedCombinations = function () {
        for (let i = 0; i < this.combinations.length; i++) {
            innerLoop: for (let course in this.combinations[i]) {
                let courseName = this.combinations[i][course].name + "-$-" + this.combinations[i][course].section;
                if (this.conflicts[courseName] != undefined) {

                    for (let j = 0; j < this.conflicts[courseName].length; j++) {
                        let conCourse = this.conflicts[courseName][j];
                        if (this.combinations[i][conCourse.name].section == conCourse.section) {
    ...
    filter = function(sections, setRoutineData){
        for(let i=0;i<this.combinations.length;i++){
            innerLoop : for(let j=0;j<sections.length;j++){
                if(this.combinations[i][sections[j].name].section != sections[j].section){
                    this.combinations.splice(i,1);
                    i--;
                    break innerLoop;
                }
