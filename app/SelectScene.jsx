      ...
      try {
        AsyncStorage.getItem('@sectionselector:data').then((value)=>{
          if(value !== null) {
            ...
            AsyncStorage.setItem('@sectionselector:data', JSON.stringify(inputs));
          }
        })
      } catch(e) {
        // error reading value
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      if (timeIsSmallerOrEqual(inputs[i][3], inputs[i][2])) {
        return {
          valid: false,
          index: [i],
          message: "End time can't be smaller or equal to start time.",
        };
      }
        ...
        <Pressable
          onPress={() => {
            ...
            if (valid.valid) {
              ...
              scrollRef.current.scrollTo({x:0, y: dataSourceCords[valid.index[0]] });
            }
          }}
